...
CREATE TABLE student_interests (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_student_interests_profile FOREIGN KEY (student_id) REFERENCES student_profiles (id) ON DELETE CASCADE
);
...
CREATE TABLE company_qualifications (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_company_qualifications_profile FOREIGN KEY (company_id) REFERENCES company_profiles (id) ON DELETE CASCADE
);
...
CREATE TABLE cases (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_cases_company FOREIGN KEY (company_id) REFERENCES company_profiles (id) ON DELETE CASCADE
);
...
CREATE TABLE case_qualifications (
    ...
    value VARCHAR(255) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_case_qualifications_case FOREIGN KEY (case_id) REFERENCES cases (id) ON DELETE CASCADE
);

-- Case offerings (what the company can offer the student)
CREATE TABLE case_offerings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    case_id INT NOT NULL,
    offering_type VARCHAR(100) NOT NULL,
    offering_value VARCHAR(255) NULL,
    CONSTRAINT fk_case_offerings_case FOREIGN KEY (case_id) REFERENCES cases (id) ON DELETE CASCADE
);

-- Notifications: generated when a published case matches a student above their threshold
CREATE TABLE notifications (
    ...
    CONSTRAINT fk_notif_case FOREIGN KEY (case_id) REFERENCES cases (id) ON DELETE CASCADE,
    UNIQUE KEY uk_notif_user_case (user_id, case_id)
);